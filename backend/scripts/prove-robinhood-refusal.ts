...
const MANDATE_ID = process.env.PROVE_MANDATE_ID ?? 'aab33461-c700-4df7-bbc2-742019d49354';
const TARGET_ADDRESS = process.env.PROVE_TARGET_ADDRESS ?? '0xf76e6B0920e9332fF4410f6dD53F01722AbC71a3';
const TICKER = process.env.PROVE_TICKER ?? 'TSLA';
const AMOUNT = process.env.PROVE_AMOUNT ?? '250';
...
async function main() {
  ...
  const idempotencyKey = `phase-d-${TICKER.toLowerCase()}-refused-${Date.now()}`;
  ...
  const risk = await pool.query(
    `SELECT score, tier, requires_approval, factor_summary FROM risk_scores WHERE execution_id = $1 ORDER BY calculated_at DESC LIMIT 1`,
    [executionId],
  );
  console.log('RISK', JSON.stringify(risk.rows[0] ?? null));

  const settlement = await pool.query(
    `SELECT id, status, tx_hash FROM settlements WHERE execution_id = $1 ORDER BY created_at DESC LIMIT 1`,
    [executionId],
  );
  ...
  if (finalExecutionStatus !== 'risk_failed' || settlement.rows.length > 0) {
    process.exitCode = 1;
  }
}
