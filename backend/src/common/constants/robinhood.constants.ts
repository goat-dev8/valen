...
export const ROBINHOOD_TESTNET_USDG = '0x7E955252E15c84f5768B83c41a71F9eba181802F' as const;
...
const TICKER_TO_ADDRESS: Record<string, `0x${string}`> = Object.fromEntries(
  ROBINHOOD_STOCK_SYMBOLS.map((symbol) => [symbol, ROBINHOOD_STOCK_TOKENS[symbol].address.toLowerCase() as `0x${string}`]),
);

export function resolveRobinhoodTickerAddress(ticker: string): `0x${string}` | null {
  return TICKER_TO_ADDRESS[ticker.toUpperCase()] ?? null;
}

export function isRobinhoodStockSymbol(value: string): value is RobinhoodStockSymbol {
  return value.toUpperCase() in ROBINHOOD_STOCK_TOKENS;
}
