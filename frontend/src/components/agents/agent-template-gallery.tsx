...
import { CloneTemplateModal, type CloneTemplateOptions } from '@/components/agents/clone-template-modal';
import {
  AGENT_TEMPLATES,
...
export function AgentTemplateGallery({ existingAgents = [] }: { existingAgents?: AgentDto[] }) {
  ...
  const cloneTemplate = cloneTemplateId ? agentTemplateById(cloneTemplateId) : null;
  const alreadyInstalled = cloneTemplate
    ? existingAgents.some((agent) => agentMatchesTemplate(agent.name, cloneTemplate))
    : false;
  ...
  const handleCloneConfirm = async (options: CloneTemplateOptions) => {
    if (!cloneTemplateId) return;
    setError(null);
    try {
      await cloneMutation.mutateAsync({ templateId: cloneTemplateId, options });
    ...
    <section className="space-y-4">
      <div>
        ...
        <p className="mt-1.5 max-w-2xl text-sm font-medium text-[#8B98A5]">
          Clone a governed agent with policy, capabilities, and activation pre-configured.
        </p>
      </div>
      ...
      <div className="agent-fleet-grid">
        {AGENT_TEMPLATES.map((template) => {
          ...
          const installed = existingAgents.some((agent) => agentMatchesTemplate(agent.name, template));
          ...
          return (
            <article key={template.id} className="agent-fleet-card agent-template-card">
              <div className="agent-fleet-card__visual">
                ...
                {installed ? (
                  <span className="agent-fleet-live agent-fleet-live--idle">Ready in Fleet</span>
                ) : (
              ...
              <div className="agent-fleet-card__body">
                ...
                <div className="agent-template-actions">
                  <button
                    ...
                    disabled={installed || cloneMutation.isPending}
                    onClick={() => setCloneTemplateId(template.id)}
                  >
                    {installed ? 'Already Installed' : 'Clone agent'}
                  </button>
                  <button type="button" className="app-btn app-btn-outline text-xs" onClick={() => handleCopy(template)}>
                    {copiedId === template.id ? <><Check className="h-3.5 w-3.5" />Copied</> : <><Copy className="h-3.5 w-3.5" />Copy setup</>}
                  </button>
                </div>
                {template.demoIntentTemplateId && (
                  <Link href={`/dashboard/executions/new?template=${template.demoIntentTemplateId}`} className="agent-template-demo-link">
      ...
      <CloneTemplateModal
        open={Boolean(cloneTemplateId)}
        template={cloneTemplate}
        alreadyInstalled={alreadyInstalled}
        pending={cloneMutation.isPending}
        onClose={() => setCloneTemplateId(null)}
